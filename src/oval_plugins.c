...
oval_drop_privileges (gpointer user_data)
{
  ...
  if (getuid () == 0)
    {
      log_write
        ("oval_plugins.c: Running as root, trying to drop privileges.\n");
      if ((nobody_pw = getpwnam ("nobody")))
        {
          if (setgid (nobody_pw->pw_gid) == 0)
            {
              log_write
                ("oval_plugins.c: Successfully dropped group privileges.\n");
            }
          else
            {
              log_write
                ("oval_plugins.c: WARNING: Could not drop group privileges!\n");
            }
          if (setuid (nobody_pw->pw_uid) == 0)
            {
              log_write
                ("oval_plugins.c: Successfully dropped user privileges.\n");
            }
          else
            {
              log_write
        ...
        {
          log_write
            ("oval_plugins.c: WARNING: Could not drop privileges; unable to get uid and gid for user nobody!\n");
        }
    }
  else
    {
      log_write
        ("oval_plugins.c: WARNING: Did not attempt to drop privileges since we do not seem to be running as root.\n");
    }
}
 ...
 */
struct arglist *
oval_plugin_add (char *folder, char *name, struct arglist *plugins,
  ...
  if (args == NULL)
    {
      ...
      if (g_slist_length (plugin_list) == 0)
        {
          log_write
            ("oval_plugin_add: Empty plugin_list, no definitions found in %s!",
             fullname);
          return NULL;
        }
...
ovaldi_launch (struct arglist *g_args)
{
  ...
  if (sc_file == NULL)
  ...
  else
    {
      ...
      while (res)
        {
          g_string_append_printf (system_data,
                                  "\t\t<inetlisteningserver_item id=\"%d\" xmlns=\"http://oval.mitre.org/XMLSchema/oval-system-characteristics-5#linux\">\n",
                                  i);
          g_string_append_printf (system_data,
              ...
              for (k = 0; k < g_strv_length (user_array); k++)
                {
                  ...
                  gchar *result =
                    kb_item_get_str (kb, g_strconcat ("USER_SID/", username,
                                                      NULL));
                  if (result == NULL)
                    {
                      log_write ("Could not get a kb_item for USER_SID/%s.",
                                 username);
                    }
                  else
                    {
                      ...
                      if (g_ascii_strcasecmp (items[1], " Enabled") == 0)
                      ...
                      else
                        {
                          if (g_ascii_strcasecmp (items[1], " Disabled") == 0)
                            ...
                            {
                              log_write
                                ("%s is neither enabled nor disabled???",
                                 username);
                            }
                        }
                      if (g_strv_length (items) > 2)
                        {
                          int j;
                          groups =
                            g_malloc0 ((g_strv_length (items) -
                                        1) * sizeof (gchar *));
                          for (j = 2; j < g_strv_length (items); j++)
                            {
                              log_write ("%s is in group %s.", username,
                                         items[j]);
                      ...
                      g_string_append_printf (system_data,
                                              "\t\t<user_item id=\"%d\" xmlns=\"http://oval.mitre.org/XMLSchema/oval-system-characteristics-5#windows\">\n",
                                              i);
                      g_string_append_printf (system_data,
                                              "\t\t\t<user>%s</user>\n",
                                              username);
                      ...
                      if (g_ascii_strcasecmp (username, "Administrator") == 0)
                        {
                          g_string_append_printf (collected_objects,
                                                  "\t\t<object flag=\"complete\" id=\"oval:gov.nist.fdcc.xp:obj:60221\" version=\"1\">\n");
                          g_string_append_printf (collected_objects,
                                                  "\t\t\t<reference item_ref=\"%d\"/>\n",
                                                  i);
                          g_string_append_printf (collected_objects,
                                                  "\t\t</object>\n");
                          collected = TRUE;
              ...
              for (k = 0; k < g_strv_length (user_array); k++)
                {
                  gchar *username = user_array[k];
                  gchar *result =
                    kb_item_get_str (kb, g_strconcat ("SID_ITEM/", username,
                                                      NULL));
                  if (result == NULL)
                    {
                      log_write ("Could not get a kb_item for SID_ITEM/%s.",
                                 username);
                    }
                  else
                    {
                      ...
                      if (g_strv_length (items) == 3)
                        {
                          g_string_append_printf (system_data,
                                                  "\t\t<sid_item id=\"%d\" xmlns=\"http://oval.mitre.org/XMLSchema/oval-system-characteristics-5#windows\">\n",
                                                  i);
                          g_string_append_printf (system_data,
                                                  "\t\t\t<trustee_name>%s</trustee_name>\n",
                                                  g_strstrip (items[0]));
                          g_string_append_printf (system_data,
                                                  "\t\t\t<trustee_sid>%s</trustee_sid>\n",
                                                  g_strstrip (items[1]));
                          g_string_append_printf (system_data,
                                                  "\t\t\t<trustee_domain>%s</trustee_domain>\n",
                                                  g_strstrip (items[2]));
                          g_string_append_printf (system_data,
                                                  "\t\t</sid_item>\n");
                          // Workaround for PoC to create a collected_objects that
                          // will make ovaldi happy
                          if (g_ascii_strcasecmp (items[0], "Administrator") ==
                              0)
                            {
                              g_string_append_printf (collected_objects,
                                                      "\t\t<object flag=\"complete\" id=\"oval:gov.nist.fdcc.xp:obj:12\" version=\"1\">\n");
                              g_string_append_printf (collected_objects,
                                                      "\t\t\t<reference item_ref=\"%d\"/>\n",
                                                      i);
                              g_string_append_printf (collected_objects,
                          ...
                          if (g_ascii_strcasecmp (items[0], "Gast") == 0)
                            {
                              g_string_append_printf (collected_objects,
                                                      "\t\t<object flag=\"complete\" id=\"oval:gov.nist.fdcc.xp:obj:6\" version=\"1\">\n");
                              g_string_append_printf (collected_objects,
                                                      "\t\t\t<reference item_ref=\"%d\"/>\n",
                                                      i);
                              g_string_append_printf (collected_objects,
                        ...
                        {
                          log_write
                            ("Expected 3 items in SID_ITEM, but found %d!",
                             g_strv_length (items));
                        }
      ...
      if (kb_item_get_str (kb, "ssh/login/release") == NULL)
        {
          log_write
            ("Could not identify release, not collecting package information.\n");
          result_string =
            g_strdup_printf
                  ...
                  while (package[j] != NULL)
                    {
                      strtok (package[j], " ");
                      g_string_append_printf (system_data,
                                              "\t\t<dpkginfo_item id=\"%d\" xmlns=\"http://oval.mitre.org/XMLSchema/oval-system-characteristics-5#linux\">\n",
                                              i);
                      g_string_append_printf (system_data,
                                              "\t\t\t<name>%s</name>\n",
                                              strtok (NULL, " "));
              ...
              if (packages_str)
                {
                  ...
                  while (package[j] != NULL)
                    {
                      gchar *pgpsig =
                        strncpy (keyid, package[j] + strlen (package[j]) - 16,
                                 16);
                      g_strchug (package[j]);
                      ...
                      if (package_data[0])
                        {
                          ...
                          g_string_append_printf (system_data,
                                                  "\t\t<rpminfo_item id=\"%d\" xmlns=\"http://oval.mitre.org/XMLSchema/oval-system-characteristics-5#linux\">\n",
                                                  i);
                          g_string_append_printf (system_data,
                                                  "\t\t\t<name>%s</name>\n",
                                                  package_name);
                          ...
                          g_string_append_printf (system_data,
                                                  "\t\t\t<evr/>\n");
                          g_string_append_printf (system_data,
                                                  "\t\t\t<signature_keyid>%s</signature_keyid>\n",
                                                  pgpsig);
                          g_string_append_printf (system_data,
                                                  "\t\t</rpminfo_item>\n");
                          i++;
      ...
      if (!g_file_get_contents (results_filename, &filebuffer, &length, NULL))
      ...
      else
        {
          ...
          if (g_slist_length (result_list) == 0)
            {
              log_write
                ("oval_result_add: Empty result_list, no results found in %s!",
                 basename);
            }
          ...
          if (g_slist_length (result_list) > 1)
            {
              gchar **result_array;
              result_array =
                g_malloc0 ((g_slist_length (result_list) +
                            1) * sizeof (gchar *));
              ...
              for (i = 0; i < g_slist_length (result_list); i++)
                {
                  ...
                  result_array[i] =
                    g_strdup_printf
                    ("The OVAL definition %s returned the following result: %s\n",
