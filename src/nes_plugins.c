...
dlopen (name)
     char *name;
{
  return (ext_library_t) shl_load (name,
                                   BIND_IMMEDIATE | BIND_VERBOSE | BIND_NOSTART,
                                   0L);
}
     ...
     char *name;
{
  ...
  if (NSCreateObjectFileImageFromFile (name, &ofile) !=
  ...
  return NSLinkModule (ofile, name,
 ...
 */
pl_class_t *
nes_plugin_init (struct arglist * prefs, struct arglist * args)
{
      ...
      else
        {
          ...
          if ((f_init = (plugin_init_t) LOAD_FUNCTION (ptr, "plugin_init"))
              || (f_init = (plugin_init_t) LOAD_FUNCTION (ptr, "_plugin_init")))
            {
              ...
              arg_add_value (args, "preferences", ARG_ARGLIST, -1,
                             (void *) preferences);
              e = (*f_init) (args);
              if (e >= 0)
                {
            ...
            log_write ("Couldn't find the entry point in %s [%s]\n", name,
                       LIB_LAST_ERROR ());
          CLOSE_LIBRARY (ptr);
        }
    }
