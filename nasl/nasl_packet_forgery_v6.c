...
get_tcp_v6_element (lex_ctxt *lexic)
{
  ...
  /* Packet must hold a full IPv6 header plus a TCP header before any of
   * its fields, including ip6_plen below, can be read safely. */
...
get_tcp_v6_option (lex_ctxt *lexic)
{
  ...
  */
  ...
  // ipv6 header = 40 + tcp header = 20 + tcp options
  if (40 + 20 + (size_t) options_len > ipsz)
    return NULL;
