...
struct TCPConnection {
    socket: TcpStream,
    _tls_connection: Option<rustls::ClientConnection>,
...
enum NaslSocket {
    // The TCP Connection is boxed, because it uses alot of space
...
impl NaslSockets {
    fn open_udp(addr: &str, port: u16) -> Result<NaslSocket, FunctionErrorKind> {
        Ok(NaslSocket::Udp(UDPConnection {
            socket: UdpSocket::bind(format!("{addr}:{port}"))?,
            buffer: vec![],
        }))
    }
    ...
    fn open_tcp(
        ...
        Ok(NaslSocket::Tcp(Box::new(TCPConnection {
            socket,
            _tls_connection: tls_connection,
            _buffer: buffer,
        })))
    }
    ...
    fn recv<K>(&self, r: &Register, _: &Context<K>) -> Result<NaslValue, FunctionErrorKind> {
        ...
        let timeout = super::get_opt_int(r, "timeout").unwrap_or_default();
        ...
        match self
            ...
            NaslSocket::Udp(conn) => {
                ...
                for _ in 0..5 {
                    ...
                    if data_available {
                        conn.socket.recv(data.as_mut_slice())?;
    ...
    fn open_sock_kdc<K>(
        ...
        let socket = match use_tcp {
            true => Self::open_tcp(&hostname, port as u16, None, None, None),
            false => Self::open_udp(&hostname, port as u16),
        }?;
    ...
    fn open_sock_tcp<K>(
        ...
        let transport = super::get_opt_int(register, "transport");
        ...
        if let Some(transport) = transport {
            if transport < 0 {
                ...
                match OpenvasEncaps::try_from(transport) {
                    // Auto Detection
                    Ok(OpenvasEncaps::Auto) => {
                        // Try SSL/TLS first
                        if let Ok(fd) =
                    ...
                    // Unsuported transport layer
                    Err(_) | Ok(OpenvasEncaps::Max) => {
                        return Err(FunctionErrorKind::WrongArgument(format!(
                            "unsupported transport layer: {transport}(unknown)"
                        )))
                    }
                    // TLS/SSL
                    Ok(tls_version) => match tls_version {
                        OpenvasEncaps::Tlsv12 => todo!(),
                        OpenvasEncaps::Tlsv13 => todo!(),
                        _ => {
                            return Err(FunctionErrorKind::WrongArgument(format!(
        ...
        Ok(NaslValue::Fork(vec![]))
    }

    fn open_sock_tcp_ip<K>(
        &self,
        context: &Context<K>,
        addr: &str,
        port: u16,
    ...
    fn open_sock_tcp_tls<K>(
        ...
        let encrypted =
            pkcs8::EncryptedPrivateKeyInfo::from_der(key.secret_der()).map_err(|_| {
                FunctionErrorKind::Diagnostic(
                    format!("Unable to decrypt private key {key_path} with given password"),
                    None,
                )
            })?;
        let decrypted = encrypted.decrypt(password).map_err(|_| {
            FunctionErrorKind::Diagnostic(
                format!("Unable to decrypt private key {key_path} with given password"),
                None,
            )
        ...
        let key = PrivateKeyDer::Pkcs8(PrivatePkcs8KeyDer::from(decrypted.as_bytes().to_owned()));
