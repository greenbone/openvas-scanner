    ...
    T: Serialize,
{
    ...
    pub fn deserialize(self) -> Result<T, base::Error> {
        match self {
            Serialization::Deserialize(s) => Ok(s),
    ...
    T: for<'de> Deserialize<'de>,
{
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        match bincode::deserialize(&value) {
            Ok(t) => Ok(Serialization::Deserialize(t)),
            Err(_) => Err(base::Error::Serialize),
        }
    }
}
...
impl<T> AsRef<[u8]> for Serialization<T> {
    fn as_ref(&self) -> &[u8] {
        match self {
            Serialization::Deserialize(_) => &[0u8],
...
impl<T> From<Serialization<T>> for Vec<u8> {
    fn from(s: Serialization<T>) -> Self {
        match s {
            Serialization::Deserialize(_) => vec![0u8],
...
mod test {
    ...
    fn serialization() {
        ...
        let t = match s {
            super::Serialization::Deserialize(t) => t,
    ...
    fn create_on_append() {
        ...
        let test2 = match results.get(0).unwrap() {
            super::Serialization::Deserialize(t) => t.clone(),
            _ => panic!("Serialization::try_from failed"),
        };
