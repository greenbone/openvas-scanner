    ...
    DB: crate::storage::Storage + std::marker::Send + 'static + std::marker::Sync,
{
    ...
    if let Some((roots, certs, key)) = tlsc {
        ...
        let config = Arc::new(tls::server_config(verifier, certs, key)?);
        ...
        loop {
            ...
            let identifier = client_identifier.clone();
