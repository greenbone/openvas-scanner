...
use futures_util::{stream::futures_unordered::IterMut, Future};
use serde_json::{to_string, Map};
use tokio::sync::RwLock;
use redis_storage::{VtHelper, RedisWrapper, RedisAddNvt, RedisAddAdvisory, RedisGetNvt};
...
pub trait GetVts {
    ...
    async fn get_vts(&self, vt_selection: Option<Vec<String>>) -> Result<Vec<String>, StorageError>;
    //Result<Box<dyn Iterator<Item = String + Send>, StorageError>> 
}
    ...
    K: AsRef<str> + Sync + Send + 'static,
{
    ...
    async fn get_vts(&self, vt_selection: Option<Vec<String>>) -> Result<Vec<String>, StorageError> {
        ...
        for oid in oids {
            nvts.push(serde_json::to_string(&self.vthelper.read().await.retrieve_single_nvt(&oid).unwrap().unwrap()).unwrap());
        }
