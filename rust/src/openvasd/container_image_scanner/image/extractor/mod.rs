...
impl Extractor {
    ...
    pub async fn locator(self) -> Vec<FileSystemLocator> {
        self.architecture
            .clone()
            .into_iter()
            .map(|arch| {
    ...
    pub async fn extract(&mut self, layer: PackedLayer) -> Result<Duration, ExtractorError> {
