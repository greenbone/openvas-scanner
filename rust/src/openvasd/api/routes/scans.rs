...
async fn get_scans_preferences() -> impl IntoResponse {
    axum::Json::from(scannerlib::scanner::preferences::preference::PREFERENCES)
}
...
async fn post_scans_id(
    ...
    // maybe change to 202 Accepted instead
...
async fn get_scans_id_results(
    ...
    let range = params.range.map(|x| parse_results_range(&x)).transpose()?;
...
pub(super) fn parse_results_range(range: &str) -> Result<(usize, usize), ApiError> {
    if let Some((start, end)) = range.split_once('-') {
            ...
            && let Ok(end) = end.parse::<usize>()
        {
            ...
            Err(ApiError::InvalidInput(
                "\"{range}\" is not a valid range".to_string(),
            ))
        }
