...
use std::{collections::HashSet, num::ParseIntError, sync::Arc};
...
use scannerlib::{models, scanner::preferences::preference::PREFERENCES};

/// Access point for all `/scans` prefixed routes.
pub fn router(
    ...
    // All routes are relative to `/scans/`
    Router::new()
        .route(
            "/",
...
/// `GET /scans/preferences` route handler.
///
/// Authenticated: yes
///
...
async fn get_scans_preferences() -> impl IntoResponse {
    Json::from(PREFERENCES)
}

/// `GET /scans` route handler.
///
/// Authenticated: yes
///
...
async fn get_scans(
    ...
    into_json_stream(scans_stream).await
}

/// `POST /scans` route handler.
///
/// Authenticated: yes
///
...
async fn post_scans(
    ...
    if let Some(dup) = get_duplicate_credential_service(&scan.target.credentials) {
        return Err(ApiError::DuplicateCredentials(dup));
    }
...
/// `GET /scans/{id}` route handler.
///
/// Authenticated: yes
///
/// Provides `JSON` encoded information about a scan with a given `id` previously added by `POST /scans`.
/// All passwords are redacted and it contains no status or result information.
///
/// ## Errors
/// * 401: unauthorized access
/// * 404: scan not be found
...
/// `DELETE /scans/{id}` route handler.
///
/// Authenticated: yes
///
/// Deletes a scan with a given ID. The scan cannot be deleted when it is currently running.
/// The current status can be retrieved via `GET /scans/{id}/status`.
///
...
/// `GET /scans/{id}/results` route handler.
///
/// Authenticated: yes
///
...
async fn get_scans_id_results(
    ...
    let range = params
        .range
        .map(|x| {
            parse_results_range(&x)
                .map_err(|_| ApiError::InvalidInput(format!("\"{x}\" is not a valid range")))
        })
        .transpose()?
        .unwrap_or((None, None));
...
/// `GET /scans/{id}/results/{rid}` route handler.
///
/// Authenticated: yes
///
...
/// * 401: unauthorized access
/// * 404: scan not found
async fn get_scans_id_results_rid(
    client_id: Extension<ClientId>,
...
/// `GET /scans/{id}/status` route handler.
///
/// Authenticated: yes
///
/// Provides `JSON` encoded status information about a scan with a given `id` previously added by `POST /scans`.
///
/// ## Schema
/// ```json
///{
...
/// Finds and returns duplicate credentials for a single service.
pub(super) fn get_duplicate_credential_service(
    credentials: &[models::Credential],
) -> Option<models::Service> {
...
/// Parses a string either containing either a single number or into an inclusive numerical range.
///
/// ## Examples
/// * `3` becomes the start and end values `3 and 3`
/// * `1 2` becomes `1 and 2` respectively
pub(super) fn parse_results_range(
    range: &str,
) -> Result<(Option<usize>, Option<usize>), ParseIntError> {
    if let Some((start, end)) = range.split_once('-') {
        Ok((Some(start.parse::<usize>()?), Some(end.parse::<usize>()?)))
    } else {
        Ok((Some(range.parse::<usize>()?), None))
    }
}
...
mod tests {
    ...
    use crate::{
        Config,
        api::{
            ApiConfig, Authentication, routes,
    ...
    /// Tests that all /scans prefixed routes require authentication.
    /// All requests are relative from the /scans prefix.
    #[tokio::test]
    ...
    ///
    /// While this test covers the usage of one out of multiple possible valid `api-keys`, it is
    /// currently not possible to set more than a single `api-key` via the config or cmdline arguments.
    #[tokio::test]
    ...
    /// Tests client data separation for the GET /scan route
    #[tokio::test]
    async fn get_scans_client_separation() -> anyhow::Result<()> {
        let cfg = config().await?;
        let mut router = routes::scans::router(
            cfg.scanner,
            Authentication::ApiKey,
        ...
        // Since the request is router relative the real API url is POST /scans
    ...
    #[test]
    fn range_parser() {
        assert_eq!(super::parse_results_range("1-3"), Ok((Some(1), Some(3))));
        assert_eq!(super::parse_results_range("3"), Ok((Some(3), None)));
        assert!(super::parse_results_range("1-a").is_err());
        assert!(super::parse_results_range("a-3").is_err());
        assert!(super::parse_results_range("a").is_err());
    }
}
