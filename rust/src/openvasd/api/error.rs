    ...
    pub fn status_code(&self) -> StatusCode {
        match self {
            ...
            ApiError::Notus(e) => match e {
                ...
                NotusError::PackageParseError(_) => StatusCode::BAD_REQUEST,
...
impl From<ApiError> for ErrorJson {
    fn from(value: ApiError) -> Self {
        ...
        let (message, details) = match value {
            ...
            ApiError::Notus(NotusError::PackageParseError(package)) => {
                ("Invalid notus package".to_string(), Some(package))
            }
            ApiError::Scheduling(_)
            | ApiError::VtsError(_)
            | ApiError::Notus(_)
            | ApiError::JoinError(_) => ("Internal server error".to_string(), None),
