...
pub struct ApiConfig {
    ...
    /// TLS server and mTLS config.
    pub tls_cfg: config::Tls,
...
pub async fn run(cfg: &ApiConfig) -> anyhow::Result<i32> {
    ...
    match (cfg.tls_cfg.certs.as_ref(), cfg.tls_cfg.key.as_ref()) {
        ...
        (Some(cert_path), Some(key_path)) => {
            ...
            let mut server_config =
                if let Some(client_certs_path) = cfg.tls_cfg.client_certs.as_ref() {
                    let verifier = mtls::create_verifier(client_certs_path)?;
        ...
        (None, None) => {
            ...
            // Start without TLS and with HTTP1.1
            axum_server::bind(cfg.address)
                .handle(shutdown_handle)
                .serve(app.into_make_service())
                .await?;
...
pub(super) mod tests {
    ...
    pub fn json_request<T: serde::Serialize>(
        ...
        Request::builder()
            ...
            .expect("failed to create valid HTTP request")
    }
}
