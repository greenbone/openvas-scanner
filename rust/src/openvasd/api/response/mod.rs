...
/// Implements the http body types
...
macro_rules! internal_server_error {
    ($e:expr) => {{
        ...
        $crate::api::response::BodyKind::no_content(axum::http::StatusCode::INTERNAL_SERVER_ERROR)
    }};
    () => {{
        $crate::api::response::BodyKind::no_content(axum::http::StatusCode::INTERNAL_SERVER_ERROR)
    }};
...
impl IntoResponse for BodyKind {
    fn into_response(self) -> Response {
        let mut resp = Response::new(axum::body::Body::new(self.content));
        *resp.status_mut() = self.status_code;

        // this struct is only used for streamed results anymore which are all JSON encoded
        resp.headers_mut().insert(
            header::CONTENT_TYPE,
            "application/json".parse().expect("valid HTTP header value"),
        );
