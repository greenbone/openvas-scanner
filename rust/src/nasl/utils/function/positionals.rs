...
use crate::nasl::{NaslError, Register};
...
impl<'a, T: FromNaslValue<'a>> Positionals<'a, T> {
    ...
    /// The item type is Result<T, NaslError>, since
    /// the conversion to T can still fail.
    pub fn iter(&self) -> impl Iterator<Item = Result<T, NaslError>> + 'a {
        self.register
            .positional()
            .iter()
...
impl<'a, T: FromNaslValue<'a>> CheckedPositionals<T> {
    ...
    pub fn new(register: &'a Register) -> Result<Self, NaslError> {
        let data = register
            .positional()
            .iter()
