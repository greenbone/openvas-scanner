...
fn sign_alg_oid_to_name(oid: &str) -> String {
    match oid {
...
fn pub_key_alg_oid_to_name(name: &str) -> String {
    match name {
...
fn subject_oid_to_name(oid: &str) -> String {
    match oid {
        "2.5.4.6" => "C",
        "2.5.4.8" => "ST",
        "2.5.4.7" => "L",
...
impl NaslCerts {
    ...
    fn key_size(cert: &X509Certificate) -> Option<i64> {
        if let Some(algorithm) = cert.key_algorithm() {
            match algorithm {
    ...
    fn cert_query(
        ...
        let result = match query {
            ...
            "subject" => Self::subject(cert, idx)
                .map(NaslValue::String)
                .unwrap_or(NaslValue::Null),
            "issuer" => Self::issuer(cert, idx)
                .map(NaslValue::String)
                .unwrap_or(NaslValue::Null),
            "not-before" => {
            ...
            "hostnames" => NaslValue::Array(
                Self::hostnames(cert)
                    .into_iter()
                    .map(NaslValue::String)
                    .collect::<Vec<NaslValue>>(),
            ),
            "image" => NaslValue::Data(cert.encode_der().unwrap_or_default()),
            "algorithm-name" | "signature-algorithm-name" => {
                let signature_algorithm =
                    sign_alg_oid_to_name(&cert.signature_algorithm_oid().to_string());
                NaslValue::String(signature_algorithm)
            }
            "public-key-algorithm-name" => {
                let public_key_algorithm =
                    pub_key_alg_oid_to_name(&cert.key_algorithm_oid().to_string());
