...
fn load_private_key(filename: &str) -> Result<PrivateKeyDer<'static>, TLSError> {
    ...
    loop {
        match rustls::pki_types::pem::from_buf(&mut reader).map_err(|e| {
            TLSError::Io(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("No private key found in {filename}: {}", e),
            ))
        })? {
            Some((SectionKind::PrivateKey, key)) => {
                return PrivateKeyDer::from_pem_slice(&key)
                    .map_err(|e| TLSError::KeyError(format!("Key error {}", e)));
            }
            None => break,
            _ => {}
        }
