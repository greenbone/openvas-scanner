...
impl TryFrom<&[u8]> for Frame {
    type Error = NaslError;

    fn try_from(f: &[u8]) -> Result<Self, Self::Error> {
        if f.len() < 14 {
            Err(NaslError::missing_argument("valid ip address"))
        } else {
...
fn validate_mac_address(v: Option<&ContextType>) -> Result<MacAddr, NaslError> {
    ...
    mac_addr.ok_or_else(|| NaslError::wrong_unnamed_argument("mac address", "invalid mac address"))
}
...
fn recv_frame(cap: &mut Capture<pcap::Active>, filter: &str) -> Result<Frame, NaslError> {
    let f = Frame::new();
...
fn nasl_send_arp_request(register: &Register, context: &Context) -> Result<NaslValue, NaslError> {
    ...
    let local_mac_address = match get_local_mac_address(&iface.name) {
        Some(x) => x,
        _ => {
            return Err(NaslError::missing_argument(
...
fn nasl_forge_frame(register: &Register, _: &Context) -> Result<NaslValue, NaslError> {
...
fn nasl_send_frame(register: &Register, context: &Context) -> Result<NaslValue, NaslError> {
...
fn nasl_dump_frame(register: &Register, _: &Context) -> Result<NaslValue, NaslError> {
