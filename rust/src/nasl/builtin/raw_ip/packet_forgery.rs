...
fn print_tcp_packet(tcp: &Option<packet::tcp::TcpPacket>) -> Result<(), FunctionErrorKind> {
    match tcp {
        Some(pkt) => {
...
fn get_ip_v6_element(
    ...
    let buf = match register.named("ip6") {
        Some(ContextType::Value(NaslValue::Data(d))) => d.clone(),
        _ => {
            return Err(FunctionErrorKind::missing_argument("ip6"));
        }
    };
...
fn dump_ip_v6_packet(register: &Register, _: &Context) -> Result<NaslValue, FunctionErrorKind> {
    let positional = register.positional();
    if positional.is_empty() {
        return Err(FunctionErrorKind::MissingPositionalArguments {
            expected: 1,
...
fn dump_tcp_v6_packet(register: &Register, _: &Context) -> Result<NaslValue, FunctionErrorKind> {
    let positional = register.positional();
    if positional.is_empty() {
        return Err(FunctionErrorKind::Dirty(
            "Missing arguments. It needs at least one tcp packet".to_string(),
...
/// Returns the modified IP datagram or NULL on error.
fn forge_udp_v6_packet(register: &Register, _: &Context) -> Result<NaslValue, FunctionErrorKind> {
    let mut ip_buf = match register.named("ip6") {
        Some(ContextType::Value(NaslValue::Data(d))) => d.clone(),
        _ => return Err(FunctionErrorKind::missing_argument("ip6")),
    };
    let original_ip_len = ip_buf.len();
...
/// Receive a list of IPv4 datagrams and print their UDP part in a readable format in the screen.
fn dump_udp_v6_packet(register: &Register, _: &Context) -> Result<NaslValue, FunctionErrorKind> {
    let positional = register.positional();
    if positional.is_empty() {
        return Err(FunctionErrorKind::Dirty(
            "Missing arguments. It needs at least one UDP packet".to_string(),
...
fn forge_icmp_v6_packet(
    ...
    let mut ip_buf = match register.named("ip6") {
        Some(ContextType::Value(NaslValue::Data(d))) => d.clone(),
        _ => {
            return Err(FunctionErrorKind::missing_argument("icmp6"));
        }
    };
...
/// Receive a list of IPv4 ICMP packets and print them in a readable format in the screen.
fn dump_icmp_v6_packet(register: &Register, _: &Context) -> Result<NaslValue, FunctionErrorKind> {
    let positional = register.positional();
    if positional.is_empty() {
        return Err(FunctionErrorKind::missing_argument("icmp"));
    }
