...
//! OpenVAS TCP Port Scanner
//!
//! This module provides a high-performance, adaptive TCP port scanner with:
...
//! - Firewall and rate-limit detection
...
fn set_rtt_stats(context: &ScanCtx<'_>, stat: &RttStats, rtt_type: &str) -> Result<(), FnError> {
    ...
    context.set_kb_item(
        ...
        KbItem::String(format!("{:.4}", stat.mean().unwrap().as_secs_f64())),
    )?;
    context.set_kb_item(
    ...
    context.set_kb_item(
        ...
        KbItem::String(format!("{:.4}", stat.max.as_secs_f64())),
    )?;
    context.set_kb_item(
    ...
    context.set_kb_item(
        ...
        KbItem::String(format!("{:.4}", stat.std_dev().unwrap())),
    )?;
    context.set_kb_item(
...
async fn plugin_run_openvas_tcp_scanner(context: &ScanCtx<'_>) -> Result<(), FnError> {
    ...
    let port_range = context.get_open_tcp_ports()?;
    ...
    for port in &results.open_ports {
        if let Some(banner) = results.banners.get(port) {
            context.set_kb_item(KbKey::Banner(*port), KbItem::Data(banner.to_owned()))?;
            context.set_kb_item(
                KbKey::Banner(*port),
                KbItem::String(format!("{:x?}", banner)),
            )?;
        } else {
