...
use nasl_c_lib::krb5::{
    ...
    OKrb5ErrorCode_O_KRB5_UNABLE_TO_WRITE, OKrb5GSSContext, OKrb5Slice, OKrb5Target, OKrb5User,
    o_krb5_add_realm, o_krb5_find_kdc, o_krb5_gss_prepare_context, o_krb5_gss_session_key_context,
    o_krb5_gss_update_context, okrb5_gss_free_context, okrb5_gss_init_context,
};
...
impl Krb5 {
    ...
    #[nasl_function]
    fn krb5_gss_init(&mut self) -> Result<(), FnError> {
    ...
    fn krb5_gss_prepare_context(
        ...
        self.last_okrb5_result =
            unsafe { o_krb5_gss_prepare_context(&credential, *cached_gss_context) };
        Ok(self.last_okrb5_result)
    }

    #[nasl_function]
    fn krb5_gss_update_context(&mut self, data: StringOrData) -> u32 {
        let data = data.data();
        let data = OKrb5Slice {
        ...
        unsafe {
            o_krb5_gss_update_context(
                *cached_gss_context,
                &data,
                &mut *to_application,
                &mut self.gss_context_needs_more,
            )
        }
    }
