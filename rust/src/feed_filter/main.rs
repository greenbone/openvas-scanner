...
fn run_builtin_filter(args: BuiltinFilterArgs) -> Result<(), CliError> {
...
fn main() -> Result<(), CliError> {
    let cli = Cli::parse();
    match cli.command {
        Command::BuiltinFilter(args) => run_builtin_filter(args),
        Command::DepFilter(args) => run_dep_filter(args),
    }
}
