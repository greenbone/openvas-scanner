// SPDX-FileCopyrightText: 2024 Greenbone AG
//
// SPDX-License-Identifier: GPL-2.0-or-later

mod helper;
#[cfg(test)]
mod tests {

    use super::helper::decode_hex;
    use nasl_interpreter::*;
    use test_utils::TestBuilder;

    #[test]
    fn rsa_public_encrypt() {
        let mut t = TestBuilder::default();
        t.run(r#"data = raw_string("Message for test case!");"#);
        t.run(r#"priv_pem = raw_string(r"-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n");"#);
        t.run(r#"n = hexstr_to_data("");"#);
        t.run(r#"e = hexstr_to_data("");"#);
        t.run(r#"d = hexstr_to_data("");"#);
        t.run(r#"enc_data = rsa_public_encrypt(data:data,n:n,e:e,pad:TRUE);"#);
        t.ok(
            r#"rsa_private_decrypt(data:enc_data,n:n,e:e,d:d,pad:TRUE);"#,
            NaslValue::Data("Message for test case!".into()),
        );
        t.run(r#"enc_data = rsa_public_encrypt(data:data,n:n,e:e,pad:FALSE);"#);
        t.ok(
            r#"rsa_private_decrypt(data:enc_data,n:n,e:e,d:d,pad:FALSE);"#,
            NaslValue::Data("Message for test case!".into()),
        );
        t.ok(r#"sign = rsa_sign(data:data,pem:priv_pem,passphrase:raw_string(""));"#,decode_hex("802D2364DC1B9A99B62AFC6E5344B5682FD7742767C42EEB90E49C60281B0475984FFFA40C68CFB61D1EFAC490D4B3282F09BE84DA781D90BB356954264107D3").unwrap());
        t.run(r#"rsa_public_decrypt(sign:sign,e:e,n:n);"#);
    }
}
