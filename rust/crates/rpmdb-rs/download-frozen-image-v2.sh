...
# This script is downloaded from: https://raw.githubusercontent.com/moby/moby/refs/heads/master/contrib/download-frozen-image-v2.sh
# and adapted to our use case for systems that don't have docker installed
...
usage() {
	echo "usage: $0 file dir image[:tag][@digest] ..."
	echo "       $0 usr/lib/sysimage/rpm/rpmdb.sqlite /tmp/old-hello-world hello-world:latest@sha256:8be990ef2aeb16dbcb9271ddfe2610fa6658d13f6dfb8bc72074cc1ca36966a7"
	[ -z "$1" ] || exit "$1"
}
...
fetch_blob() {
	...
	curl -L -S "${curlArgs[@]}" \
		...
		-o "$targetFile" \
		-D-
	echo "extracting $targetFile to $dir"
	# ok this downloads json as well
	if [[ "layer.tar" = "$(basename $targetFile)" ]]; then
		tar tf $targetFile \
		| grep "$extract_file" \
		| tar --transform='s,^/,,' -xf $targetFile -C $dir -T - || \
		echo "$targetFile does not contain $extract_file"
