...
pub trait GetScansIDResultsID: ContainsScanID {
...
pub struct GetScansIDResultsIDIncomingRequest<T> {
    get_scans: Arc<T>,
}
        ...
        'b: 'a,
    {
        ...
        Box::pin(async move {
            ...
            enforce_client_id_and_scan_id(&client_id, id, gsp.as_ref(), async |client_id, id| {
                match gsp.get_scans_id_results_id(client_id, id, rid).await {
...
impl<T> From<T> for GetScansIDResultsIDIncomingRequest<T>
where
    T: GetScansIDResultsID + 'static,
{
    fn from(value: T) -> Self {
        GetScansIDResultsIDIncomingRequest {
            get_scans: Arc::new(value),
        }
    }
}

impl<T> From<Arc<T>> for GetScansIDResultsIDIncomingRequest<T>
where
    T: GetScansIDResultsID + 'static,
{
    fn from(value: Arc<T>) -> Self {
        GetScansIDResultsIDIncomingRequest { get_scans: value }
    }
}
...
mod tests {
    ...
    impl GetScansIDResultsID for Test {
        fn get_scans_id_results_id(
            ...
            Box<dyn Future<Output = Result<models::Result, GetScansIDResultsIDError>> + Send>,
        > {
    ...
    async fn internal_server_error() {
        let entry_point = test_utilities::entry_point(
            Authentication::MTLS,
            incoming_request!(GetScansIDResultsIDIncomingRequest::from(Test {})),
