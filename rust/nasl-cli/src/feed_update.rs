...
pub fn run(storage: &dyn Sink, path: PathBuf, verbose: bool) -> Result<(), CliError> {
    ...
    let loader: FSPluginLoader = root_dir.as_path().try_into()?;
    ...
    // load feed version
    let code = loader.load("plugin_feed_info.inc")?;
    let mut register = Register::default();
    let mut interpreter = Interpreter::new("inc", storage, &loader, &mut register);
    for stmt in nasl_syntax::parse(&code) {
        retry_interpret(&mut interpreter, &stmt?)?;
    }
    let feed_version = register
        .named("PLUGIN_SET")
        .map(|x| x.to_string())
        .unwrap_or_else(|| "0".to_owned());
    retry_dispatch(storage, "generic", NVTField::Version(feed_version).into())?;
    ...
    for entry in WalkDir::new(path).into_iter().filter_map(|e| e.ok()) {
        ...
        if matches!(ext.as_str(), "nasl") {
            let code = FSPluginLoader::load_non_utf8_path(entry.path())?;
...
fn execute_description_run(
    ...
    Err(CliError::NoExitCall(key.to_owned()))
}
