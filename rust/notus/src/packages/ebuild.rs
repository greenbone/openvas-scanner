...
#[cfg(test)]
mod tests {
    use std::{fs::File, io::{self, BufRead}, path::PathBuf, cmp::Ordering};
    ...
    pub fn test_comparability() {
        ...
        let apache2 = EBuild::from_name_and_full_version(
        ...
        assert!(apache2.clone().unwrap().
                compare(apache1.clone().unwrap())
                .unwrap().is_gt());

        assert!(apache1.clone().unwrap().
                compare(apache2.clone().unwrap())
                .unwrap().is_le());

        let apache3 = EBuild::from_name_and_full_version(
            "www-servers/apache", "2.4.51-r3"
        );
        ...
        let apache4 = EBuild::from_name_and_full_version(
            "apache", "2.4.51-r3"
        );
        assert!(apache4.is_some());
        assert!(apache4.unwrap().
                   compare(apache3.unwrap())
                   .unwrap().is_ne()
        );
    }
}
