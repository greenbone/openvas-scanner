...
/// NASL function to create a list out of a number of unnamed arguments
fn make_list<K, S>(register: &Register, _: &Context<K, S>) -> Result<NaslValue, FunctionErrorKind> {
...
fn nasl_sort<K, S>(register: &Register, _: &Context<K, S>) -> Result<NaslValue, FunctionErrorKind> {
...
fn keys<K, S>(register: &Register, _: &Context<K, S>) -> Result<NaslValue, FunctionErrorKind> {
    let positional = resolve_positional_arguments(register);
...
fn max_index<K, S>(register: &Register, _: &Context<K, S>) -> Result<NaslValue, FunctionErrorKind> {
...
pub(crate) fn lookup<K, S>(key: &str) -> Option<NaslFunction<K, S>> {
    match key {
